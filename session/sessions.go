...
// // User can have several uuid session, one for each browser, divice e etc...
// var userIdByUUIDSession map[uuidsession]userid = make(map[uuidsession]userid)

// // Each use have only one session
// var sesssionByUserID map[userid]*SessionData = make(map[userid]*SessionData)

type session struct {
	db           sqlx.DB
	userIDs      map[uuidsession]userid
	sessionsData map[userid]*SessionData
}

func NewSession(db sqlx.DB) *session {
	return &session{
		db:           db,
		userIDs:      make(map[uuidsession]userid),
		sessionsData: make(map[userid]*SessionData),
	}
}
...
func (s *session) CreateSession(w http.ResponseWriter, userID int) error {
	// create cookie
	sUUID := uuid.NewV4()
	...
	// Save on cache.
	userIdByUUIDSession[sUUIDString] = userID
	return nil
}
...
func (s *Sessions) RemoveSession(w http.ResponseWriter, req *http.Request) {
	...
	if err == http.ErrNoCookie {
		...
		delete(userIdByUUIDSession, c.Value)
	}
}
...
func (s *Sessions) getUserIdfromSessionUUID(req *http.Request) (int, error) {
	...
	if cookie != nil {
		...
		userID := userIdByUUIDSession[sessionUUID]
		// Found on cache.
		if userID != 0 {
		...
		if userID != 0 {
			...
			userIdByUUIDSession[sessionUUID] = userID
			return userID, nil
		}
	}
	// No cookie
...
func (s *Sessions) getSessionFromUserID(userID int) (session *SessionData, err error) {
	// From the cache.
	session = sesssionByUserID[userID]
...
func (s *Sessions) cacheSession(userID int) (session *SessionData, err error) {
	...
	// Cache it.
	sesssionByUserID[userID] = session
	return session, nil
}

// CleanSessions clean the cache session.
func (s *Sessions) CleanSessions() {
	sesssionByUserID = map[int]*SessionData{}
